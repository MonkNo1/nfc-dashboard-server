...
const generateRandomSlug = (): string => crypto.randomBytes(8).toString('hex').toLowerCase();
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.error("Error saving slug profile:", error instanceof Error ? error.message : 'Unknown error');
    ...
    console.error("Slug generation error:", error instanceof Error ? error.message : 'Unknown error');
    return res.status(500).json({ error: 'Internal server error' });
  }
});
